# -*- coding: utf-8 -*-
"""
Created on Tue Nov  6 21:32:26 2018
...
def parse_args():
    parser=argparse.ArgumentParser(description='A simple image data augmentation tool!',
    ...
    parser.add_argument('num',help='Number of images to be augmented images',type=int)
    ...
    parser.add_argument('--p_crop',help='Ratio to random crop an image',type=float,default=1.0)
    parser.add_argument('--crop_size',help='the ratio of cropped image size to orginal image size,in area',
                        type=float,default=0.9)    
    parser.add_argument('--crop_hw_vari',help='Variation of h/w ratio',type=float,default=0.1)    
    parser.add_argument('--p_rotate',help='Ratio to random rotate an image',type=float,default=1.0)    
    parser.add_argument('--p_rotate_crop',help='Ratio to  crop out the empty part in a rotated image',
                        type=float,default=1.0)
    parser.add_argument('--rotate_angle_vari',help='Variation range of rotate angle',type=float,default=10.0)    
    parser.add_argument('--p_hsv',help='Ratio to change gamma of an image',type=float,default=1.0)    
    ...
    parser.add_argument('--p_gamma',help='Ratio to random change gamma of an image',type=float,default=1.0)    
...
'''
根据进程数和要增加的图片数，生成每个进程要处理的文件列表和每个文件要增加的数目 
'''
def generate_image_list(args):
    ...
    img_list=[(os.sep.join([args.input_dir,filename]),num_ave_aug+1 if lucky else num_ave_aug) for filename,
                           lucky in zip(filenames,lucky_seq)]
    ...
    for filepath,n in filelist:
        ...
        for i in range(n):
            ...
            if random.random()<args.p_crop:
                img_varied=ia.random_crop(img_varied,args.crop_size,args.crop_hw_vari)
                varied_imgname+='c'
            if random.random()<args.p_rotate:
                img_varied=ia.random_ratate(img_varied,args.rotate_angle_vari,args.p_rotate_crop)
                varied_imgname+='r'                
...
def main():
    ...
    print('Starting image data augmentation for {} with \n {}\n'.format(args.input_dir,params_str))
